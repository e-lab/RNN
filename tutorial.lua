...
require('image')
require('base')
local datain = require('data-tutorial')
require('sys')
-- require('env')
...
--rnn_size      (default 100)     RNN number of neurons / LSTM cells
--dropout       (default 0)       dropout parameter
--init_weight   (default 0.04)    initial weights RNN
--lr            (default 1)       learning rate
--vocab_size    (default 100)     vocabolary size for words dictionary
--max_epoch     (default 2)       lower learning rate every max_epoch epochs
--max_max_epoch (default 4)       max epochs in training loop
--max_grad_norm (default 10)      max gradient normalization
--trainsize     (default 40)      train set size
--testsize      (default 10)      test set size
--valsize       (default 10)      validation set size
...
local function run_test()
  ...
  for i = 1, (len - 1) do
    ...
    print('Testing: word in, out:', datain.vocab.words[x[1]], datain.vocab.words[y[1]])
  end
...
local function main()
  ...
  -- load dataset:
  local dstrain, dstest,dsval = datain.get_dataset(opt.batch_size, opt.trainsize, opt.testsize, opt.valsize)
